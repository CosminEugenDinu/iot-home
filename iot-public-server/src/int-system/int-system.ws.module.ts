...
import { KEvent, KWsClient, TParsedMsgHandler } from "./int-system.ws.types";
type THandlerId = string;

export class IntegrationSystemWsModule extends ModuleBase {
  ...
  async init() {
    ...
    wss.on("connection", (ws, req) => {
      ...
      if (req.headers[KWsHeader.WS_CLIENT_NAME] === KWsClient.RPI_LOCAL_INTEGRATION_SYSTEM) {
        this.sysClient = ws;
